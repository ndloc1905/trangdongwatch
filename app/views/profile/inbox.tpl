		...
		<div class="tabNewsH">
			<div class="inTabNewsH">
				...
				<tr>
					...
					<td>
						<a href="javascript:void(0);" onclick="deleteInbox();" class="bntS1"><span>Xóa Thư</span></a>
					</td>
				</tr>
					...
					<td>
						...
						*}
						...
						<ul style="width:700px;">
							{*
							<li>
								<a href="{$smarty.const.HOST}/{$row.susername}">{$row.susername}</a>
							</li>
							*}
							<li><a href="{$smarty.const.HOST}/{$row.susername}"><strong>{$row.susername}</strong></a>: <a href="{$smarty.const.HOST_VIEWMAIL}?id={$row.id}" class="gray1{if $row.isread==0} bold{/if}">{$row.subject}</a> - <a href="{$smarty.const.HOST_VIEWMAIL}?id={$row.id}">Trả lời</a></li>
				...
				{else}
					<div class="listEmpty">Hiện tại, Bạn chưa có thư đến.</div>
				{/if}
			</div>
		</div>
	</div>
