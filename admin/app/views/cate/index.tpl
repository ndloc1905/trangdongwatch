...
<form onsubmit="return false;" id="form1" name="form1" method="POST" action="{$smarty.const.HOST_CATE}/doaction">
<div class="padT2">
	...
	{select_cateparent item0=$t->_('all') filter='1' selected=$intParentId attribs='onchange="subcate(this.value);" style="float:left;margin-right:2px;"'}
...
<table width="100%" cellpadding="0" cellspacing="0" border="1">
	<tr>
		...
		<th>{$t->_('parent_name')}</th>			
		<th width="120px">{$t->_('zorder')}</th>
		<th width="80px">{$t->_('status')}</th>			
		<th width="50px">{$t->_('edit')}</th>
	</tr>
	{foreach from=$arrResult item=row key=i}
	<tr>
		<td><input type="checkbox" name="items[]" value="{$row.cateid}" /></td>
		<td>{$row.name}</td>			
		<td>{$row.parent_name}</td>
		<td align="center">{$row.zorder}</td>
		<td align="center">{$row.status|status}</td>
		<td align="center"><a href="{$smarty.const.HOST_CATE}/update?id={$row.cateid}">{$t->_('edit')}</a></td>
	</tr>
	{/foreach}
	<tr>
...
<div class="padT2">
	<input type="button" value="{$t->_('addnew')}" onclick="document.location.href='{$smarty.const.HOST_CATE}/update'" class="btnAction" />
