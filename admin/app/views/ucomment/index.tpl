...
<form onsubmit="return false;" id="form1" name="form1" method="POST" action="{$smarty.const.HOST_UCOMMENT}/doaction">
<div class="padT2">
...
<table width="100%" cellpadding="0" cellspacing="0" border="1">
	...
	<tr>
		...
		<td>{$row.username}</td>
		<td>{$row.rusername}</td>
		<td>{$row.content}<br/>{$row.postdate|date_format:"%H:%M %d/%m/%Y"}</td>		
		<td align="center">{$row.status|status}</td>
		<td align="center"><a href="{$smarty.const.HOST_UCOMMENT}/update?id={$row.id}">{$t->_('edit')}</a></td>
	</tr>
	{/foreach}
	<tr>
